...
void test_copyConstructor() {
    fun<int(void)> b(bar);
    fun<int(void)> second(b);
    assert(second() == 2);
}

void test_nullptrConstructor() {
    fun<void(void)> f(nullptr);
}

void test_moveConstructor() {
    fun<int(void)> b(bar);
    fun<int(void)> second(std::move(b));
    assert(second() == 2);
}
...
void test_moveAssignment() {
    fun<int(void)> b(bar);
    fun<int(void)> second(foo);
    second = std::move(b);
    assert(second() == 2);
}

void test_explicitOperatorBool() {
    fun<int(void)> f(nullptr);
    assert(!f);
    f = foo;
    assert(f);
}
...
void NIKITOZZZZ_test() {
    ...
    fun<int (std::ostream &)> f([=](std::ostream &os) mutable {
